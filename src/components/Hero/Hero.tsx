...
export const Hero: React.FC<HeroProps> = ({ onOpenConversation, onNavigate }) => {
  ...
  const bgMediaRef = useRef<HTMLVideoElement>(null);

  useEffect(() => {
    if (bgMediaRef.current) {
      bgMediaRef.current.play().catch(() => {});
    }
    ...
    const ctx = gsap.context(() => {
      ...
      // Background video settles subtly
      tl.fromTo(
        bgMediaRef.current,
  ...
  return (
    <section id="hero" ref={sectionRef} className="hero-editorial-section">
      {/* Full-bleed Cinematic Background Video Loop */}
      <div className="hero-editorial-bg-wrap">
        <video
          ref={bgMediaRef}
          className="hero-editorial-bg-video"
          autoPlay
          loop
          muted
          playsInline
